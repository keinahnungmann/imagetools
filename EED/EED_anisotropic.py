...
def EED(image, plicit, alpha, tau, h1, h2, sigma, rho, quantile, iterations):
    ...
    for z in range(iterations):
        ...
        for k in range(K):
            ...
            J_k = [cv2.GaussianBlur(elem, (rho, rho), 0) for elem in outer_k] 
            J += J_k
        ...
        for i in range(w):
            for j in range(h):
                    ...
                    matrix = np.stack(([J[0][j,i], J[1][j,i]], [J[1][j,i], J[2][j,i]]))
                    ...
                    #compute diffusivity: 
                    #norm of regularised gradient magnitude
                    aux = np.sqrt((matrix[0,0] - matrix[1,1])**2 + 4*matrix[1,0]**2)
                    e1 = 0.5*(matrix[0,0] + matrix[1,1] - aux)
                    edger = abs(e1)
...
def main(input_path, output_folder, *args):
    ...
    processed_image = EED_anisotropic.EED(image, *args)
