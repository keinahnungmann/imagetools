...
def CED(image, plicit, alpha, tau, h1, h2, sigma, rho, quantile, iterations):
    ...
    #contrast parameter	
    C = 1
    #split original image, iterator curr
    RGB_image = cv2.split(image)	#split original image in channels for reaction equation
    curr = image
    h,w,_ = image.shape
    ...
    for z in range(iterations):
        ...
        #TODO: compute .7 quantile of eigenvalues of structure tensor as contrast parameter
        ...
        for i in range(w):
            for j in range(h):
                ...
                coh = (e0 - e1)**2
                lam1 = 0.0001
                lam2 = 0.0001 + 0.9999*np.exp(-C/coh)
        ...
        for i in range(w):
            for j in range(h):
                beta[j,i] = (19/32)*(1 if J[1][j,i] > 0 else -1 if J[1][j,i] < 0 else 0.0)
        ...
        for k in range(K):
            if alpha > 0:
                    ...
                    vector = 1 / (alpha+tau) * (alpha*RGB[k] + tau*RGB_image[k])	#is only a convex combination if tau, alpha < 1
                    RGB[k] = sp.linalg.cg(matrix, vectorize(vector), maxiter=10000, tol=10e-6)[0].astype(np.uint8).reshape(h,w)
                   
            else:
                    matrix = I - tau*A
                    RGB[k] = sp.linalg.cg(matrix, vectorize(RGB[k]), maxiter=10000, tol=10e-6)[0].astype(np.uint8).reshape(h,w)        
